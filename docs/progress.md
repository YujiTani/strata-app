...
### 2026-07-28（Week 3 / 第6日）

- **やったこと**:
  - **`docs/db-map.md` を新設**。既存の`data-model.md`は「なぜこう持つか」の設計意図の文書で、
    ER図もmermaidのため手元で図として読めていなかった。「いまDBがどういう状態か」を掴むための
    実務用の地図（ASCIIの全体地図・テーブル別カード・売買時に動く4テーブルのスナップショット・
    状態確認SQL）を別に起こした。
  - **`listings` → `market_listings` にリネーム**。名前から「何の掲示か」が読み取れないため改名。
    アプリ側の参照は`schema.ts`の1行のみで無傷だった。
  - `drizzle-kit generate`は**リネームを差分から自動判定できず、対話で create / rename を聞いてくる**
    ことを実地で確認。createを選ぶとDROP+CREATEになりデータが全損する分岐がここにある。
    renameを選び、生成SQLが`ALTER TABLE ... RENAME TO`で始まることを確認してから適用した。
  - FK制約名はdrizzleが自動でDROP→ADDして付け替えるため手作業不要だった（旧名が残る想定は外れ）。
  - **本番マイグレーションのsecretの向き先を検証・クローズ**。`0002`のpushで
    production branchにも`market_listings`が反映され、GitHub Actionsの`DATABASE_URL_UNPOOLED`が
    production branchを指していることを確認できた（7/27の積み残し）。
- **判断/詰まったこと**:
  - 「CIがsuccessした」ではなく「**production branchに実際にテーブルが存在する**」で確認した。
    7/24に踏んだ「CI成功 ≠ 実機反映」の罠を、今回は意識的に回避できた。
  - `ALTER TABLE ... ADD CONSTRAINT FOREIGN KEY`は既存行の検証のため参照元・参照先の
    両テーブルにロックを取る。行数が多い本番では書き込みが止まる＝**マイグレーション自体が
    ロック事象**である、という視点を得た（今回は0行のため無害）。
- **次**: Q2（`FOR UPDATE`無しで二重販売が成立するタイムライン）を書く。
- **所要時間**: 約2h（目安）

### 2026-07-30（Week 3 / 第7日）

- **やったこと**:
  - **Q2を完成**（`docs/week3-q2-schedule.md`）。`FOR UPDATE`無しの購入処理で二重販売が成立する
    スケジュール（＝直列化可能でないスケジュール）を、19ステップのインターリーブ図として書き切った。
    自力で骨格まで書き、レビューを4往復して仕上げた。
  - **ADR-0012「マーケット購入の同時実行制御」を起こした**。決定は
    (1) アプリ層は`SELECT ... FOR UPDATE`で行を確保してから判断、
    (2) DB層は`purchases`テーブル新設＋`UNIQUE(market_listing_id)`で二重販売を物理的に拒否、
    (3) `purchases.id`を`ledger_entries.ref_id`に使う、
    (4) ロック順序は`market_listings → item_instances → players → wallets`、
    同一テーブル内は`player_id`昇順。捨てた選択肢5案（条件付きUPDATE / アプリ内排他 / 楽観ロック /
    SERIALIZABLE）とその理由も記録。
  - **`market_listings`が素材（`inventory_stacks`）を出品できないことを発見**。`spec.md`は
    「自給自足できない**素材**を売買する市場」を経済ループの中心に置いているのに、
    `market_listings.item_instance_id`はNOT NULLで1点ものしか指せない。**仕様とスキーマの食い違い**。
    → Week 3は1点もののみで通し、素材出品は積み残しとして明示的に記録する判断（案a）。
- **判断/詰まったこと**:
  - **待たされた`UPDATE`は救いにならない**（今回の最大の学び）。ロック待ちが解けた`UPDATE`は
    最新の確定版を読み直して`WHERE`を再評価するが（EvalPlanQual）、`WHERE id = ?`しか
    書いていないため条件は必ずマッチし、**古い判断のまま実行される**。
    判断条件（`status='active'`）はアプリの`if`の中にあり、DBからは見えない。
    → **アプリの`if`に書いた条件は、DBにとって存在しない。**
  - **危険な窓は「①と③の間」ではなく「①を読んでからCOMMITするまで」全体**。READ COMMITTEDでは
    未COMMITの変更が他Txから見えないため、Aの COMMIT 直前にBが読んでも`active`が返る。
    差し込み位置を全候補試して、壊れないのは「AがCOMMITした後にBが始まる」場合だけだった。
  - **保存則チェックでは事故を検出できない**。二重販売が成立しても通貨総量は保存され、
    `wallets`と`SUM(ledger_entries)`も一致し、`item_instances.owner_id`も1人のまま。
    壊れるのは「代金を払った人が現物を受け取る」という、テーブルの形に表現されていない約束。
    → **「整合性チェックが通ること」は「壊れていないこと」を意味しない。**
    唯一の物証は`ledger_entries`が1取引あたり4行になること。
  - `FOR UPDATE`が効く理由を「割り込みされにくい」（確率の話）ではなく、
    **「待った後に最新の確定値を返すので、ロック取得後に判断材料を読み直せる」**と言い直した。
    → 先に素の`SELECT`で読み、後から`FOR UPDATE`を足すのは無意味。順序が本質。
  - ロック順序について、当初「買い手→売り手」でよいと考えたが、**役割ベースの順序は破れる**。
    アオイがベンから買うのと同時にベンがアオイから買うと逆順になる。役割は相対的な位置なので、
    値そのもの（`player_id`昇順）を基準にする必要がある。
  - `UNIQUE`は単一の値だけでなく**組み合わせ**も禁止できる、と理解が更新された
    （`daily_claims`の`UNIQUE(player_id, claim_date)`）。あわせて
    **主キーを含めた複合UNIQUEは常に成立するので意味がない**ことも確認。
- **次**:
  - ADR-0012の検収（「理由」「結果・トレードオフ」を自分の言葉で説明できるか）
  - `POST /market_listings`（出品）→ 購入トランザクション（まず`FOR UPDATE`無し）
  - 並行購入スクリプトで二重販売を再現し、紙の予言が現実に再現されるか確認
  - `ORDER BY ... FOR UPDATE`でロック取得順が保証されるかを実験で確定
  - 既存tick実装のロック順序がADR-0012の宣言順序と矛盾しないかコードで確認
- **所要時間**: 約3h（目安）
